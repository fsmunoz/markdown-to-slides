...
module.exports = function (argv, md) {
	...
	if (argv['embed-images']) {
		...
		// For images redefine the render function to include the base64 representation
		renderer.image = function(href, title, text) {
			// Add error checking? The default error
			// message seems clear enough.  This only
			// accounts for local reads and must be
			// changed to include all possible URIs.
			var bitmap = fs.readFileSync(href);
			var base64Image = new Buffer(bitmap).toString('base64');
			return '![' + text + '](' + 'data:image;base64,' + base64Image + (title ? ' "' + title + '"' : '') + ')';
		}
		var parser = new marked.Parser({renderer: renderer});
		md = parser.parse(tokens);
	}
